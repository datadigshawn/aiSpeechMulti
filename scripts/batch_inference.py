...
更新紀錄（2026.1..27)
自動修正 batch_inference.py 的參數問題
修正 auto_config → auto_convert_audio + use_config_manager

待確認修正與否
修正 1 (必須): 移除 Chirp 3 不支援的講者識別參數
修正 2 (建議): 讓 transcribe_file 接受 **kwargs
修正 3 (可選): 詞彙表搜尋路徑優化
修正 4 (建議): 錯誤處理增強
...
def interactive_mode():
    ...
    # ========================================================================
    # 步驟 3.5: 選擇區域（僅 Google STT）
    # ========================================================================
    stt_region = None  # 預設值

    if model_type == 'google_stt':
        print()
        print("【步驟 3.5/4】選擇 Google STT 服務區域")
        print("-" * 70)
        print("1. us (美國) - 推薦，所有模型可用")
        print("   - 優點: 完整支援 Chirp 2/3")
        print("   - 缺點: 延遲較高（約 200ms）")
        print()
        print("2. eu (歐洲)")
        print("   - 優點: 支援 Chirp 2/3")
        print("   - 缺點: 延遲較高")
        print()
        print("3. asia-southeast1 (新加坡)")
        print("   - 優點: 延遲最低")
        print("   - 缺點: Chirp 2/3 不可用，僅支援 Long/Short 嗎？")
        print()
        
        while True:
            region_choice = input("請選擇區域 [1/2/3，直接Enter使用us]: ").strip()
        ...
        stt_region = region_map.get(region_choice, 'us')
        
        print(f"✅ 已選擇區域: {stt_region}")
    ...
    def _init_google_stt(self):
        ...
        try:
            ...
            return GoogleSTTModel(
                ...
                auto_convert_audio=True,      # 自動轉換音訊格式
                use_config_manager=True       # 使用配置管理器
            )
...
使用範例:
  ...
  python scripts/batch_inference.py --test-case Test_02_TMRT --model whisper
    ...
    parser.add_argument(
        "--stt-model",
        choices=["chirp_3", "chirp_telephony", "chirp_2", "chirp", "long", "short", "telephony"],
        default="chirp_3",
